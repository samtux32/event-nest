...
// ── Vendor Follow-Up Reminder ──────────────────────────────────

export async function sendVendorFollowUpEmail({ vendorEmail, vendorName, customerName, inquiryType }) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  await send({
    to: vendorEmail,
    subject: `Reminder: ${escapeHtml(customerName)} is waiting for your response`,
    html: base(`
      <h2 style="margin:0 0 8px">You have an unanswered inquiry</h2>
      <p style="color:#6b7280;margin:0 0 24px">Hi ${escapeHtml(vendorName || 'there')}, ${escapeHtml(customerName)} sent you ${inquiryType === 'booking' ? 'a booking request' : 'a message'} over 24 hours ago and hasn't received a response yet.</p>
      <p style="color:#374151;margin:0 0 24px">Responding quickly increases your chances of getting booked. Customers often reach out to multiple vendors — being first to reply makes a big difference.</p>
      ${btn('View Your Messages', `${appUrl}/messages`)}
    `),
  })
}
...
export async function sendWaitlistConfirmationEmail({ recipientEmail, userType }) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'
  const isVendor = userType === 'vendor'
  await send({
    to: recipientEmail,
    subject: "You're on the Event Nest waitlist!",
    html: base(`
      <h2 style="margin:0 0 8px">You're on the list!</h2>
      <p style="color:#6b7280;margin:0 0 24px">Thanks for joining the Event Nest waitlist${isVendor ? ' as a vendor' : ''}. We'll let you know as soon as we launch so you can ${isVendor ? 'start receiving bookings' : 'start planning your perfect event'}.</p>
      <p style="color:#374151;margin:0 0 24px">In the meantime, follow us on social media for updates and early access perks.</p>
      ${btn('Visit Event Nest', appUrl)}
    `),
  })
}
