...
export async function sendBookingCancelledEmail({ recipientEmail, recipientName, otherPartyName, eventType, eventDate }) {
  await send({
    to: recipientEmail,
    subject: `Booking cancelled by ${otherPartyName}`,
    html: base(`
      <h2 style="margin:0 0 8px">Booking Cancelled</h2>
      <p style="color:#6b7280;margin:0 0 24px">${otherPartyName} has cancelled a booking.</p>
      <table style="width:100%;border-collapse:collapse">
        ${eventType ? `<tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Event type</td><td style="padding:8px 0;font-weight:600">${eventType}</td></tr>` : ''}
        ${eventDate ? `<tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Date</td><td style="padding:8px 0;font-weight:600">${eventDate}</td></tr>` : ''}
      </table>
...
export async function sendNewReviewEmail({ vendorEmail, vendorName, customerName, rating, reviewText }) {
  ...
  await send({
    ...
    html: base(`
      ...
      <p style="color:#6b7280;margin:0 0 24px">${customerName} has left you a review on Event Nest.</p>
      <table style="width:100%;border-collapse:collapse">
      ...
      ${reviewText ? `<p style="margin:16px 0;padding:16px;background:#f9fafb;border-radius:8px;color:#374151;font-size:14px">"${reviewText.slice(0, 200)}${reviewText.length > 200 ? '…' : ''}"</p>` : ''}
...
export async function sendVendorApprovedEmail({ vendorEmail, vendorName, profileUrl }) {
  await send({
    ...
    html: base(`
      ...
      <p style="color:#6b7280;margin:0 0 24px">Great news, ${vendorName}! Your profile has been reviewed and approved. Customers can now find and book you on Event Nest.</p>
...
export async function sendEventReminderEmail({ recipientEmail, recipientName, otherPartyName, eventType, eventDate, daysUntil }) {
  ...
  await send({
    to: recipientEmail,
    subject: `Your event with ${otherPartyName} is in ${daysUntil} days`,
    html: base(`
      ...
      <table style="width:100%;border-collapse:collapse">
        <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">With</td><td style="padding:8px 0;font-weight:600">${otherPartyName}</td></tr>
        ${eventType ? `<tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Event type</td><td style="padding:8px 0;font-weight:600">${eventType}</td></tr>` : ''}
        ${eventDate ? `<tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Date</td><td style="padding:8px 0;font-weight:600">${eventDate}</td></tr>` : ''}
      </table>
...
export async function sendReviewRequestEmail({ recipientEmail, recipientName, otherPartyName, eventType, eventDate, reviewUrl, isVendor }) {
  ...
  await send({
    to: recipientEmail,
    subject: `Share your feedback — review ${otherPartyName} on Event Nest`,
    html: base(`
      <h2 style="margin:0 0 8px">How was your experience?</h2>
      <p style="color:#6b7280;margin:0 0 24px">We'd love to hear your feedback about ${otherPartyName}. Your review helps them improve and helps other customers make informed decisions.</p>
      <table style="width:100%;border-collapse:collapse">
        ${eventType ? `<tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Event type</td><td style="padding:8px 0;font-weight:600">${eventType}</td></tr>` : ''}
        ${eventDate ? `<tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Date</td><td style="padding:8px 0;font-weight:600">${eventDate}</td></tr>` : ''}
      </table>
...
export async function sendWelcomeEmail({ recipientEmail, recipientName, isVendor }) {
  ...
  await send({
    ...
    html: base(`
      <h2 style="margin:0 0 8px">Welcome${recipientName ? `, ${recipientName}` : ''}!</h2>
...
export async function sendAccountDeletedEmail({ recipientEmail, recipientName }) {
  await send({
    ...
    html: base(`
      ...
      <p style="color:#6b7280;margin:0 0 24px">Hi${recipientName ? ` ${recipientName}` : ''}, your Event Nest account and all associated data have been permanently deleted. We're sorry to see you go.</p>
